...
import com.ibm.mqtt.callback.MqttAdvancedCallback;
import com.ibm.mqtt.callback.MqttSimpleCallback;
import com.ibm.mqtt.exception.MqttBrokerUnavailableException;
...
import com.ibm.mqtt.trace.MQeTraceHandler;
...
public class MqttClient extends MqttBaseClient implements IMqttClient {
	...
	public static final IMqttClient createMqttClient(String s,
			MqttPersistence mqttpersistence) throws MqttException {
	...
	public MqttClient(String s) throws MqttException {
		this(s, null);
	}
	...
	protected void initialise(String s, MqttPersistence mqttpersistence)
		...
		try {
			traceClass = Class
					.forName("com.ibm.mqtt.trace.MQeTraceToBinaryFile");
		...
		if (!flag)
			try {
				...
				MqttException mqttexception = new MqttException(
						"Cannot locate a J2SE Socket or J2ME StreamConnection class");
	...
	private Class loadLocalBindings() throws MqttException {
		...
		if (!flag)
			try {
				...
				MqttException mqttexception = new MqttException(
						"LocalBinding unavailable: Microbroker classes not found");
				mqttexception.initCause(classnotfoundexception1);
				throw mqttexception;
			}
		return class1;
	...
	private void start(Class class1) throws MqttException {
		try {
	...
	public void startTrace() throws MqttException {
		if (traceClass != null) {
			...
			throw new MqttException(
					"Trace classes (com.ibm.mqtt.trace.*) not found.\nCheck they are in wmqtt.jar.");
		}
	}
	...
	public void registerAdvancedHandler(
	...
	public void connect(String s, boolean flag, short word0)
			...
			MqttBrokerUnavailableException, MqttNotConnectedException {
		connect(s, flag, word0, null, 0, null, false);
	}

	public void connect(String s, boolean flag, short word0, String s1, int i,
			String s2, boolean flag1) throws MqttException,
			MqttPersistenceException, MqttBrokerUnavailableException,
		...
		if (!isAppConnected || !isSocketConnected()) {
			...
			case 1: // '\001'
				...
				throw new MqttException(
						"WMQTT protocol name or version not supported:"
								+ mqttconnect.ProtoName + " Version:"
								+ mqttconnect.ProtoVersion);
			...
			case 3: // '\003'
				throw new MqttBrokerUnavailableException(
						"WMQTT Broker is unavailable");

			default:
				tcpipDisconnect(true);
				throw new MqttNotConnectedException("WMQTT " + msgTypes[2]
						+ " not received");
			}
		}
	}
	...
	public int publish(String s, byte abyte0[], int i, boolean flag)
			throws MqttNotConnectedException, MqttPersistenceException,
			MqttException, IllegalArgumentException {
	...
	protected void publishArrived(String s, byte abyte0[], int i, boolean flag)
		...
		MQeTrace.trace(this, (short) -30009, 1048580L,
				Integer.toString(s.length()), s2,
				Integer.toString(abyte0.length), s1);
		if (simpleCallbackHandler != null)
	...
	public int subscribe(String as[], int ai[])
			throws MqttNotConnectedException, MqttException,
		...
		if (as.length != ai.length)
			throw new IllegalArgumentException("Array lengths unequal. Topics:"
					+ as.length + ", QoS:" + ai.length);
		for (int i = 0; i < as.length; i++)
			if (as[i] == null)
				throw new IllegalArgumentException(
						"NULL topic in topic array at index " + i);
	...
	public int unsubscribe(String as[]) throws MqttNotConnectedException,
			MqttException, IllegalArgumentException {
	...
	private void invalidApiInvocation() throws MqttException {
		throw new MqttException(
				"MqttClient API called in a callback method! Use a different thread.");
	}
