...
import com.ibm.mqtt.ds.MqttByteArray;
import com.ibm.mqtt.ds.MqttHashTable;
...
public abstract class MqttBaseClient extends Mqtt implements Runnable {
	...
	protected abstract void publishArrived(String s, byte abyte0[], int i,
	...
	protected void connect(String s, boolean flag, boolean flag1, short word0,
			String s1, int i, String s2, boolean flag2) throws MqttException,
	...
	private void doConnect(MqttConnect mqttconnect, boolean flag, short word0)
		...
		if (word0 > 0) {
			MqttRetry mqttretry = new MqttRetry(this, new MqttPingreq(),
					word0 * 1000);
		...
		if (!flag && persistenceLayer != null && !isConnectionLost()) {
			byte abyte0[][] = persistenceLayer.getAllSentMessages();
			if (abyte0 != null) {
				...
				for (int i = 0; i < abyte0.length; i++)
					try {
						...
						MqttRetry mqttretry1 = null;
						switch (MqttPacket.getMsgType(abyte2[0])) {
						case 3: // '\003'
							MqttPublish mqttpublish1 = new MqttPublish(abyte2,
									i1);
							vector.addElement(new Integer(mqttpublish1
									.getMsgId()));
							mqttretry1 = new MqttRetry(this, mqttpublish1,
									retryPeriod);
							break;
						...
						case 6: // '\006'
							...
							mqttretry1 = new MqttRetry(this, mqttpubrel,
									retryPeriod);
							break;
						...
						if (mqttretry1 == null)
							continue;
						synchronized (outstandingQueue) {
							outstandingQueue.put(mqttretry1.getMsgId(),
									mqttretry1);
						}
				...
				for (int j = 0; j < abyte1.length; j++)
					try {
						...
						if (MqttPacket.getMsgType(abyte3[0]) == 3) {
							MqttPublish mqttpublish = new MqttPublish(abyte3,
									j1);
							qos2PubsArrived.put(
									Integer.toString(mqttpublish.getMsgId()),
									mqttpublish);
						} else {
							invalidReceivedMessageRestored(j);
						}
	...
	protected byte[] getReturnedQoS(int i) {
		MqttByteArray mqttbytearray = (MqttByteArray) grantedQoS
				.remove(new Integer(i));
		return mqttbytearray != null ? mqttbytearray.getByteArray() : null;
	}
	...
	public void process(MqttConnack mqttconnack) {
		MQeTrace.trace(this, (short) -30017, 2097152L, new Integer(
				mqttconnack.returnCode));
		super.process(mqttconnack);
		notifyAck(1, mqttconnack.returnCode);
	}

	public void process(MqttPuback mqttpuback) {
		MQeTrace.trace(this, (short) -30018, 2097152L,
				new Integer(mqttpuback.getMsgId()));
		messageAck(mqttpuback.getMsgId());
	}

	public void process(MqttPubcomp mqttpubcomp) {
		MQeTrace.trace(this, (short) -30019, 2097152L,
				new Integer(mqttpubcomp.getMsgId()));
	...
	public void process(MqttPublish mqttpublish) {
		...
		MQeTrace.trace(this, (short) -30020, 2097152L, Integer
				.toString(mqttpublish.getMsgId()), Integer.toString(mqttpublish
				.getQos()), (new Boolean(mqttpublish.isRetain())).toString(),
				Integer.toString(i));
		if (mqttpublish.getQos() != 2)
			try {
				publishArrived(mqttpublish.topicName, mqttpublish.getPayload(),
						mqttpublish.getQos(), mqttpublish.isRetain());
			} catch (Exception exception) {
				flag = true;
				System.out.println("publishArrived Exception caught (QoS "
						+ mqttpublish.getQos() + "):");
				exception.printStackTrace();
			}
		if (mqttpublish.getQos() > 0 && !flag) {
			MQeTrace.trace(this, (short) -30021, 2097152L,
					mqttpublish.getQos() != 1 ? "PUBREC" : "PUBACK",
					new Integer(mqttpublish.getMsgId()));
					...
					if (persistenceLayer != null)
						try {
							synchronized (persistenceLayer) {
								...
								if (mqttpublish.getPayload() != null)
									abyte0 = MqttUtils.concatArray(abyte0,
											mqttpublish.getPayload());
								persistenceLayer.addReceivedMessage(
										mqttpublish.getMsgId(), abyte0);
							}
						} catch (MqttPersistenceException mqttpersistenceexception) {
							throw mqttpersistenceexception;
						} catch (Exception exception1) {
							throw new MqttPersistenceException(
									"process(MqttPublish) - packet.toBytes() failed - msgid "
											+ mqttpublish.getMsgId());
						}
					qos2PubsArrived.put(
							Integer.toString(mqttpublish.getMsgId()),
							mqttpublish);
	...
	public void process(MqttPubrec mqttpubrec) {
		MQeTrace.trace(this, (short) -30022, 2097152L,
				new Integer(mqttpubrec.getMsgId()));
		MqttRetry mqttretry = (MqttRetry) outstandingQueue.get(mqttpubrec
				.getMsgId());
	...
	public void process(MqttPubrel mqttpubrel) {
		boolean flag = false;
		MQeTrace.trace(this, (short) -30023, 2097152L,
				new Integer(mqttpubrel.getMsgId()));
		MqttPublish mqttpublish = (MqttPublish) qos2PubsArrived.get(Integer
				.toString(mqttpubrel.getMsgId()));
		if (mqttpublish != null)
			try {
				publishArrived(mqttpublish.topicName, mqttpublish.getPayload(),
						mqttpublish.getQos(), mqttpublish.isRetain());
		...
		if (!flag) {
			...
			try {
				if (persistenceLayer != null)
					synchronized (persistenceLayer) {
						persistenceLayer.delReceivedMessage(mqttpubrel
								.getMsgId());
					}
			} catch (MqttPersistenceException mqttpersistenceexception) {
	...
	public void process(MqttSuback mqttsuback) {
		MQeTrace.trace(this, (short) -30024, 2097152L,
				new Integer(mqttsuback.getMsgId()));
		grantedQoS.put(new Integer(mqttsuback.getMsgId()), new MqttByteArray(
				mqttsuback.TopicsQoS));
		messageAck(mqttsuback.getMsgId());
	}

	public void process(MqttUnsuback mqttunsuback) {
		MQeTrace.trace(this, (short) -30025, 2097152L,
				new Integer(mqttunsuback.getMsgId()));
	...
	protected int publish(String s, byte abyte0[], int i, boolean flag)
		...
		MQeTrace.trace(this, (short) -30026, 2097152L, new Integer(j),
				new Integer(i), new Boolean(flag));
		return j;
	}
	...
	protected int subscribe(String as[], int ai[]) throws MqttException {
		int i = nextMsgId();
	...
	protected int unsubscribe(String as[]) throws MqttException {
		int i = nextMsgId();
	...
	private void sendPacket(MqttPacket mqttpacket) throws MqttException,
				...
				try {
					synchronized (persistenceLayer) {
						...
						if (mqttpacket.getPayload() != null)
							abyte0 = MqttUtils.concatArray(abyte0,
									mqttpacket.getPayload());
						persistenceLayer.addSentMessage(mqttpacket.getMsgId(),
								abyte0);
					}
				} catch (MqttPersistenceException mqttpersistenceexception) {
					throw mqttpersistenceexception;
					...
					throw new MqttPersistenceException(
							"sendPacket - toBytes failed, msgid "
									+ mqttpacket.getMsgId());
				}
			boolean flag = getKeepAlivePeriod() <= 0 ? outstandingQueue.size() > 0
					: outstandingQueue.size() > 1;
			if (flag)
				l = 0L;
